...
```ts
// src/instrumentation-client.ts — primary client activation (Next.js 15+)
import { initMocksClient } from "@/mocks"
if (process.env.NEXT_PUBLIC_API_MOCKING === "enabled") {
    await initMocksClient()
}

// src/instrumentation.ts — server activation
...
// app/layout.tsx — fallback (singleton guard mencegah double init)
if (process.env.NEXT_PUBLIC_API_MOCKING === "enabled") {
