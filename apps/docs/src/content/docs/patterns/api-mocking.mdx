...
| 4 | `src/mocks/index.ts` | Daftarkan handler; `initMocksClient()` + `initMocksServer()` tersedia |
| 5 | `src/instrumentation-client.ts` + `src/instrumentation.ts` | Aktivasi primary (Next.js 15+); atau `app/layout.tsx` sebagai fallback |
...
### Aktivasi Development — Primary (`src/instrumentation-client.ts` + `src/instrumentation.ts`)

```ts
// src/instrumentation-client.ts — client, dieksekusi sekali saat app dimuat
...
```ts
// src/instrumentation.ts — server, dieksekusi saat Next.js server start
...
### Aktivasi Development — Fallback (`app/layout.tsx`)

```ts
// Singleton guard mencegah double init jika instrumentation-client.ts sudah jalan
if (process.env.NEXT_PUBLIC_API_MOCKING === "enabled") {
