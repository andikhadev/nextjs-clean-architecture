...
- [x] Tulis `llms.txt` — ringkasan convention (untuk quick context)
- [x] Tulis `llms-full.txt` — versi lengkap dengan semua contoh kode (Pattern A–H)
- [x] Tempatkan di `apps/docs/public/` agar accessible di root URL
- [ ] Verifikasi `[domain]/llms.txt` bisa diakses setelah deploy
- [x] Tambahkan link di homepage docs
- [x] Tambahkan meta tag `<link rel="llms" href="/llms.txt">` di `<head>`
